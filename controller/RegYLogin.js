...
RegYLogin.post("/login", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .render("iniciosSesion", { error: "Error interno del servidor." });
  }
});
...
RegYLogin.post("/registro", async (req, res) => {
  try {
    ...
    if (creado) {
      ...
      res.render("registrarse", { error: "Usuario o Email ya estan en uso." });
    }
  } catch (error) {
