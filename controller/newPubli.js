...
newPubli.post("/p", async (req, res) => {
  try {
    if (!req.session?.usuario) {
      return res
        .status(401)
        .json({ message: "Tenes que iniciar sesion para poder publicar." });
    }
    const {
      title,
      descripcion,
      ...
      for (let i = 0; i < img.length; i++) {
        ...
        async function ponerMarcaAgua(buffer, textoMarcaAgua) {
          try {
            ...
            img.print(fuente, 0, 0, textoMarcaAgua);
  ...
  try {
    ...
    const publis = publicaciones.map((instancia) => {
      ...
      publi.imagenes = (publi.Imagens || []).map((imgInstancia) => {
        ...
        const base64 =
          "data:image/jpeg;base64," + bufferCrudo.toString("base64");

        let votoUsuario = 0;
        if (idUsuario && imgInstancia.Valoracion) {
          const valoracion = imgInstancia.Valoracion.find(
            (v) => v.UsuarioId === idUsuario,
          );
  ...
  try {
    ...
    if (post.UsuarioId !== IdUsuario) {
      return res
        .status(403)
        .json({
          message: "No tenes los permisos para modificar esta publicaion.",
        });
    }
